...
class ChatRepo {
  ...
  Future<List<Rooms>?> fetchRooms(String email) async {
    ...
    if (response.statusCode == 200) {
      ...
      logger.w("Failed to load the rooms info: ${response.statusCode}");
      return [];
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('announcementRoom')) {
          ...
          logger.d("Announcement data: $announcement");
          Rooms announcement1 = Rooms.fromJson(announcement);
          announcements.add(announcement1);
        }
      }
    } catch (e) {
      logger.e("An error occurred while fetching announcement: $e");
    }
  ...
  Future<List<RoomMemberModel>> fetchTotalActiveMember(String roomId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.w("Failed to load users, status code: ${response.statusCode}");
        return [];
      }
    } catch (error) {
      logger.e("Error occurred while fetching users: $error");
      return [];
    }
  }
}
